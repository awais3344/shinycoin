...
{
    Object result;
    result.push_back(Pair("hash", block.GetIDHash().GetHex()));
    ...
    if (blockindex->pprev)
        result.push_back(Pair("previousblockhash", blockindex->pprev->GetBlockIDHash().GetHex()));
    if (blockindex->pnext)
...
Value stop(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "stop\n"
            "Stop shinycoin server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "shinycoin server stopping";
}
...
Value getinfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("powheight",     (int)pindexBest->nPoWHeight));
    obj.push_back(Pair("moneysupply",   ValueFromAmount(pindexBest->nMoneySupply)));
    obj.push_back(Pair("postotalmint",  ValueFromAmount(pindexBest->nPoSTotalMint)));
    obj.push_back(Pair("posdebt",       ValueFromAmount(pindexBest->nPoSDebt)));
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new shinycoin address for receiving payments.  "
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current shinycoin address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <shinycoinaddress> <account>\n"
            "Sets the account associated with the given address.");

    CBitcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid ShinyCoin address");
...
Value settxfee(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1 || AmountFromValue(params[0]) < MIN_TX_FEE)
        throw runtime_error(
            "settxfee <amount>\n"
            "<amount> is a real and is rounded to 0.001 (milli)\n"
            "Minimum and default transaction fee per KB is 1 cent");

    nTransactionFee = AmountFromValue(params[0]);
    nTransactionFee = (nTransactionFee / (CENT / 10)) * (CENT / 10);  // round to milli
    return true;
}
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 4))
        throw runtime_error(
            "sendtoaddress <shinycoinaddress> <amount> [comment] [comment-to]\n"
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <shinycoinaddress> <message>\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <shinycoinaddress> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <shinycoinaddress> [minconf=1]\n"
            "Returns the total amount received by <shinycoinaddress> in transactions with at least [minconf] confirmations.");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 3 || params.size() > 6))
        throw runtime_error(
            "sendfrom <fromaccount> <toshinycoinaddress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001\n"
            "requires wallet passphrase to be set with walletpassphrase first");
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() < 3 || params.size() > 6))
        throw runtime_error(
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid shinycoin address:")+s.name_);
...
Value listsinceblock(const Array& params, bool fHelp)
{
    ...
    else
    {
        ...
        lastblock = block ? block->GetBlockIDHash() : 0;
    }
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; shinycoin server stopping, restart to run with encrypted wallet";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <shinycoinaddress>\n"
            "Return information about <shinycoinaddress>.");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "ShinyCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "ShinyCoin is downloading blocks...");
...
TxInfoType GetInfoType(const std::string &str) {
    if (str == "normal") return INFO_NORMAL;
    if (str == "once") return INFO_WRITE_ONCE;
    if (str == "unique") return INFO_UNIQUE;
    if (str == "id") return INFO_ID;
    
    throw JSONRPCError(-32602, "Info type is one of 'normal', 'once', 'unique', or 'id'");
}

Value settxinfos(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 4 || (params.size() - 1) % 3 != 0))
        throw runtime_error(
            "settxinfos <shinycoinaddress> <key_type1> <key_string1> <value_string1> [<key_type2> <key_string2> ...]\n"
            "<key_type> is one of 'normal', 'once', 'unique', 'id'.\n"
            "<key_string> is the key name. Use key_type='id', key_string='n' for the address name\n"
            "requires wallet passphrase to be set with walletpassphrase first");
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() < 4 || (params.size() - 1) % 3 != 0))
        throw runtime_error(
            "settxinfos <shinycoinaddress> <key_type> <key_string> <value_string> [<key_type2> <key_string2> ...]\n"
            "<key_type> is one of 'normal', 'once', 'unique', 'id'.\n"
            "<key_string> is the key name. Use 'n' for the address name");
    ...
    vector<CTxInfo> infos;
    
    for (int i=1; i < params.size(); i += 3) {
        TxInfoType keyType = GetInfoType(params[i].get_str());
        string keyString = params[i+1].get_str();
        string valueString = params[i+2].get_str();
        
        infos.push_back(CTxInfo(TxInfoKey(keyType, keyString), valueString));
    }
    
    vector<pair<CBitcoinAddress, vector<CTxInfo> > > vecTxInfos;
    
    vecTxInfos.push_back(make_pair(address, infos));
    
    CWalletTx wtxNew;
    CReserveKey reservekey(pwalletMain);
    int64 nFeeRequired;
    bool fCreated = pwalletMain->CreateTransaction(vector<pair<CScript, int64> >(), vecTxInfos, wtxNew, reservekey, nFeeRequired);
    if (!fCreated)
    {
    ...
    if (!ThreadSafeAskFee(nFeeRequired, _("Sending...")))
        throw JSONRPCError(-4, "Fee not paid");
    
    if (!pwalletMain->CommitTransaction(wtxNew, reservekey))
        throw JSONRPCError(-4, "The transaction was rejected.");
    
    MainFrameRepaint();
    return wtxNew.GetHash().GetHex();
}

Value settxinfo(const Array& params, bool fHelp) {
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 4 || params.size() > 4))
        throw runtime_error(
            "settxinfo <shinycoinaddress> <key_type> <key_string> <value_string>\n"
            "<key_type> is one of 'normal', 'once', 'unique', 'id'.\n"
            "<key_string> is the key name. Use key_type='id', key_string='n' for the address name\n"
            "requires wallet passphrase to be set with walletpassphrase first");
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() < 4 || params.size() > 4))
    ...
    return settxinfos(params, fHelp);
}

Value dumptxinfos(const Array& params, bool fHelp) {
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "dumptxinfos\n"
            "Print all latest txinfos in the blockchain");
    
    ptxinfoStore->DumpLatestInfos();
...
{ //  name                      function                 safe mode?
  //  ------------------------  -----------------------  ----------
    ...
    { "settxinfos",             &settxinfos,             false },
    { "settxinfo",              &settxinfo,              false },
    { "dumptxinfos",            &dumptxinfos,            true },
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: shinycoin-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
void ThreadRPCServer2(void* parg)
{
    ...
    if (mapArgs["-rpcpassword"] == "")
    {
        ...
        string strWhatAmI = "To use shinycoind";
