...
CTxInfoStore *ptxinfoStore = NULL;
...
uint256 hashGenesisMerkle = hashGenesisMerkleOfficial;
...
unsigned int nPoSMinHeight = POS_MIN_HEIGHT;
...
const string strMessageMagic = "ShinyCoin Signed Message:\n";
...
bool CTransaction::CheckTransaction() const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, vin) {
        ...
        if (!txin.CheckTxInfos()) {
            return DoS(10, error("CTransaction::CheckTransaction() : invalid infos"));
        }
    }
    if (vout.empty() && !haveDbs)
...
bool CTxMemPool::checkAccept(CTxDB& txdb, CTransaction &tx, bool fCheckInputs, bool* pfMissingInputs,
                             CTransaction **pptxOld) {
    if (pfMissingInputs)
    ...
    if (fCheckInputs)
    {
        ...
        if (nFees < tx.GetMinFee(1000, GMF_RELAY))
...

        {
            ...
            if (!tx.ProcessTxInfos(txdb)) {
                return error("CTxMemPool::accept() : ProcessTxInfos failed");
            }
            txInfoView.Rollback();
        }
    }
...
{
    CTransaction *ptxOld;
    if (!checkAccept(txdb, tx, fCheckInputs, pfMissingInputs, &ptxOld)) {
        return false;
    }
...
int CTxIndex::GetDepthInMainChain() const
{
    ...
    map<uint256, CBlockIndex*>::iterator mi = mapBlockIndex.find(block.GetIDHash());
    if (mi == mapBlockIndex.end())
        return 0;
    CBlockIndex* pindex = (*mi).second;
    if (!pindex || !pindex->IsInMainChain())
...
uint256 CBlock::GetPoWHash() const {
    return ShinyHash(BEGIN(nVersion), END(nNonce));
}
...
bool CBlock::ReadFromDisk(const CBlockIndex* pindex, bool fReadTransactions)
{
    ...
    if (!ReadFromDisk(pindex->nFile, pindex->nBlockPos, fReadTransactions))
        return false;
    if (GetIDHash() != pindex->GetBlockIDHash())
...
int64 GetNextProofOfWorkReward(const CBlockIndex* pPrev, bool fAccountForPoS)
{
    ...
    int64 nPoWHeight = (int64)pPrev->nPoWHeight + 1;
    int64 nFullSubsidy = (200 * COIN * 52560 * 52560) / (nPoWHeight * nPoWHeight);
    nFullSubsidy = min(nFullSubsidy, MAX_MINT_PROOF_OF_WORK);

    if (!fAccountForPoS) {
        return nFullSubsidy;
    }

    int64 nSubsidy = max(nFullSubsidy / 4, nFullSubsidy - pPrev->nPoSDebt);

    if (fDebug && GetBoolArg("-printcreation")) {
        printf("GetNextProofOfWorkReward() : create=%s nHeight=%d nPoWHeight=%d, nPoSDebt=%s, nFullSubsidy=%s\n",
               FormatMoney(nSubsidy).c_str(), pPrev->nHeight + 1, pPrev->nPoWHeight + 1,
               FormatMoney(pPrev->nPoSDebt).c_str(),
               FormatMoney(nFullSubsidy).c_str());
    }

    return nSubsidy;
}

// ppcoin: miner's coin stake is rewarded based on coin age spent (coin-seconds)
int64 GetProofOfStakeReward(int64 nCoinAgeSeconds)
{
    static int64 nRewardCoinYear = CENT;  // creation amount per coin-year

    int64 nSubsidy = nRewardCoinYear * nCoinAgeSeconds * 33 / 60 / 60 / 24 / (365 * 33 + 8);
    if (fDebug && GetBoolArg("-printcreation"))
        printf("GetProofOfStakeReward(): create=%s nCoinAgeSeconds=%"PRI64d"\n",
               FormatMoney(nSubsidy).c_str(), nCoinAgeSeconds);
    return nSubsidy;
}

static const int64 nPoWTargetTimespan = 7 * 24 * 60 * 60;  // one week
static const int64 nPoSTargetTimespan = 3 * 24 * 60 * 60;  // three days
static const int64 nTargetSpacingWorkMax = 48 * STAKE_TARGET_SPACING; // 2-hour
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    if (pindexPrev->pprev == NULL)
        return (fProofOfStake ? bnInitialPoSTarget : bnProofOfWorkLimit).GetCompact(); // first block
    const CBlockIndex* pindexPrevPrev = GetLastBlockIndex(pindexPrev->pprev, fProofOfStake);
    if (pindexPrevPrev->pprev == NULL)
        return (fProofOfStake ? bnInitialPoSTarget : bnProofOfWorkLimit).GetCompact(); // second block
    ...
    int64 nInterval = (fProofOfStake ? nPoSTargetTimespan : nPoWTargetTimespan) / nTargetSpacing;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    printf("InvalidChainFound: invalid block=%s  height=%d  trust=%s\n", pindexNew->GetBlockIDHash().ToString().substr(0,20).c_str(), pindexNew->nHeight, CBigNum(pindexNew->bnChainTrust).ToString().c_str());
...
{
    int64 nBaseFee = (mode == GMF_RELAY) ? BASE_RELAY_TX_FEE : BASE_TX_FEE;
    ...
    nMinFee += CountInfoTxs() * TX_INFO_FEE;
    ...
    nMinFee = std::max(nMinFee, (mode == GMF_RELAY) ? MIN_RELAY_TX_FEE : MIN_TX_FEE);
...
bool CTransaction::_procUndoTxInfos(CTxDB &txdb, const bool undoing) {
    if (!ptxinfoStore->InTransaction()) {
        throw new std::runtime_error("Proc info without store in transaction");
    }

    for (int i=undoing ? (vin.size()-1) : 0; undoing ? (i >= 0) : (i < vin.size()); undoing ? (i--) : (i++)) {
        ...
        if (!ExtractAddress(prevTx.vout[txin.prevout.n].scriptPubKey, addr)) {
            return false;
        }

        for (int j=undoing ? (txin.infos.size()-1) : 0; undoing ? (j >= 0) : (j < txin.infos.size()); undoing ? (j--) : (j++)) {
            const CTxInfo &txinfo = txin.infos[j];
            std::string reason;
            if (!(undoing ? ptxinfoStore->Undo(addr, txinfo, reason) : ptxinfoStore->Process(addr, txinfo, reason))) {
                return error(("ProcessTxInfos(): Could not process <%s: %s>: " + reason).c_str(),
                             txinfo.key.ToString().c_str(), txinfo.value.c_str());
            }
        }
    }
...
bool CTransaction::UndoTxInfos(CTxDB &txdb) {
    return _procUndoTxInfos(txdb, true);
}
bool CTransaction::ProcessTxInfos(CTxDB &txdb) {
    return _procUndoTxInfos(txdb, false);
}
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    if (IsProofOfStake() && pindex->nHeight < nPoSMinHeight) {
        return DoS(10, error("Invalid proof-of-stkae block too early"));
    }
    ...
    BOOST_FOREACH(CTransaction& tx, vtx)
    {
        ...
        else
        {
            ...
            // (BIP16) Add in sigops done by pay-to-script-hash inputs;
            ...
            if (!tx.ProcessTxInfos(txdb)) {
                return error("ConnectBlock() : ProcessTxInfos failed");
            }
        }
    ...
    {
        CDiskBlockIndex blockindexPrev(pindex->pprev);
        blockindexPrev.hashNext = pindex->GetBlockIDHash();
        if (!txdb.WriteBlockIndex(blockindexPrev))
...
bool Reorganize(CTxDB& txdb, CBlockIndex* pindexNew)
{
    ...
    printf("REORGANIZE: Disconnect %i blocks; %s..%s\n", vDisconnect.size(), pfork->GetBlockIDHash().ToString().substr(0,20).c_str(), pindexBest->GetBlockIDHash().ToString().substr(0,20).c_str());
    printf("REORGANIZE: Connect %i blocks; %s..%s\n", vConnect.size(), pfork->GetBlockIDHash().ToString().substr(0,20).c_str(), pindexNew->GetBlockIDHash().ToString().substr(0,20).c_str());
    ...
    BOOST_FOREACH(CBlockIndex* pindex, vDisconnect)
    {
        ...
        if (!block.DisconnectBlock(txdb, pindex))
            return error("Reorganize() : DisconnectBlock %s failed", pindex->GetBlockIDHash().ToString().substr(0,20).c_str());
    ...
    for (unsigned int i = 0; i < vConnect.size(); i++)
    {
        ...
        if (!block.ConnectBlock(txdb, pindex))
        {
            // Invalid block
            txdb.TxnAbort();
            return error("Reorganize() : ConnectBlock %s failed", pindex->GetBlockIDHash().ToString().substr(0,20).c_str());
        }
    ...
    if (!txdb.WriteHashBestChain(pindexNew->GetBlockIDHash()))
...
bool CBlock::SetBestChain(CTxDB& txdb, CBlockIndex* pindexNew)
{
    ...
    nTransactionsUpdated++;
    printf("SetBestChain: new best=%s  height=%d  powheight=%d  trust=%s  moneysupply=%s  posmint=%s  posdebt=%s\n",
           hashBestChain.ToString().substr(0,20).c_str(), nBestHeight, pindexBest->nPoWHeight,
           bnBestChainTrust.ToString().c_str(),
           FormatMoney(pindexBest->nMoneySupply).c_str(), FormatMoney(pindexBest->nPoSTotalMint).c_str(),
           FormatMoney(pindexBest->nPoSDebt).c_str());
...
bool CTransaction::GetCoinAge(CTxDB& txdb, uint64& nCoinAgeSeconds) const
{
    ...
    CBigNum bnCoinSecond = bnCentSecond * CENT / COIN;
    if (fDebug && GetBoolArg("-printcoinage"))
        printf("coin age bnCoinSecond=%s=~%.5f coin days\n", bnCoinSecond.ToString().c_str(),
               (double)bnCoinSecond.getuint64() / 60 / 60 / 24);
    nCoinAgeSeconds = bnCoinSecond.getuint64();
    return true;
}

// ppcoin: total coin age spent in block, in the unit of coin-days.
bool CBlock::GetCoinAge(uint64& nCoinAgeSeconds) const
{
    nCoinAgeSeconds = 0;
    ...
    {
        uint64 nTxCoinAgeSeconds;
        if (tx.GetCoinAge(txdb, nTxCoinAgeSeconds))
            nCoinAgeSeconds += nTxCoinAgeSeconds;
        else
            return false;
    }

    if (nCoinAgeSeconds == 0) // block coin age minimum 1 coin-day
        nCoinAgeSeconds = 1;
    if (fDebug && GetBoolArg("-printcoinage"))
        printf("block coin age total nCoinSeconds=%"PRI64d"\n", nCoinAgeSeconds);
    return true;
}
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    if (miPrev != mapBlockIndex.end())
    {
        ...
        pindexNew->nPoWHeight = pindexNew->pprev->nPoWHeight + (IsProofOfWork() ? 1 : 0);
    }
    ...
    if (pindexNew == pindexBest)
    {
        ...
        static uint256 hashPrevBestCoinStake;
        UpdatedTransaction(hashPrevBestCoinBase);
        UpdatedTransaction(hashPrevBestCoinStake);
        hashPrevBestCoinBase = vtx[0].GetHash();
        if (IsProofOfStake() ) {
            hashPrevBestCoinStake = vtx[1].GetHash();
        }
    }
...
bool CBlock::CheckBlock(bool fCheckPoWHash) const
{
    ...
    for (unsigned int i = 0; i < vWorkQueue.size(); i++)
    {
             ...
             ++mi)
        {
            ...
            if (pblockOrphan->AcceptBlock())
                vWorkQueue.push_back(pblockOrphan->GetIDHash());
            mapOrphanBlocks.erase(pblockOrphan->GetIDHash());
...
bool CBlock::SignBlock(const CKeyStore& keystore)
{
    ...
    if (whichType == TX_PUBKEY)
    {
        ...
        if (key.GetPubKey() != vchPubKey)
            return false;
        return key.Sign(GetIDHash(), vchBlockSig);
    }
    return false;
}
...
bool CBlock::CheckBlockSignature() const
{
    ...
    if (whichType == TX_PUBKEY)
    {
        ...
        if (vchBlockSig.empty())
            return false;
        return key.Verify(GetIDHash(), vchBlockSig);
    }
    return false;
}
...
unsigned int CBlock::GetStakeEntropyBit() const
{
    ...
    {
        nEntropyBit = ((GetIDHash().Get64()) & 1llu);// last bit of block hash
        if (fDebug && GetBoolArg("-printstakemodifier"))
            printf("GetStakeEntropyBit(v0.4+): nTime=%u hashBlock=%s entropybit=%d\n", nTime, GetIDHash().ToString().c_str(), nEntropyBit);
    }
    else
    {
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "ShinyCoin", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        hashGenesisMerkle = hashGenesisMerkleTestNet;
        ...
        nModifierInterval = MODIFIER_INTERVAL_TEST;
        nPoSMinHeight = POS_TEST_MIN_HEIGHT;
    }
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char *pszTimestamp = fTestNet ? "Times UK 20-MAY-2014 Sri Lanka win showdown with Alex Hales NASDAQ 4.096,89 28,92" 
                                            : "Times UK 20-MAY-2014 Sri Lanka win showdown with Alex Hales DOW 16.374,31 -137,55";

        CTransaction txNew;
        txNew.nTime = fTestNet ? 1400620261 : 1400620263;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nNonce = fTestNet ? 75 : 14;

        //// debug print
        printf("%s\n", block.GetIDHash().ToString().c_str());
        printf("%s\n", hashGenesisBlock.ToString().c_str());
        printf("%s\n", block.hashMerkleRoot.ToString().c_str());
        assert(block.GetIDHash() == hashGenesisBlock);
        assert(block.hashMerkleRoot == hashGenesisMerkle);
        block.print();
        assert(block.CheckBlock(true));
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        vector<CInv> vInv;
                        vInv.push_back(CInv(MSG_BLOCK, GetLastBlockIndex(pindexBest, false)->GetBlockIDHash()));
    ...
    else if (strCommand == "getblocks")
    {
        ...
        {
            if (pindex->GetBlockIDHash() == hashStop)
            {
                printf("  getblocks stopping at %d %s (%u bytes)\n", pindex->nHeight, pindex->GetBlockIDHash().ToString().substr(0,20).c_str(), nBytes);
            ...
            if (--nLimit <= 0 || nBytes >= SendBufferSize()/2)
            {
                ...
                pfrom->hashContinue = pindex->GetBlockIDHash();
                break;
            }
        }
    }
    ...
    else if (strCommand == "getheaders")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            ...
            if (--nLimit <= 0 || pindex->GetBlockIDHash() == hashStop)
                break;
        }
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    if (pindexPrev->nHeight < nPoSMinHeight) {
        fProofOfStake = false;
    }
    ...
    if (pblock->IsProofOfWork()) {
        pblock->vtx[0].vout[0].nValue = nFees + GetNextProofOfWorkReward(pindexPrev, true);
    }
    else {
        if (nFees) {
    ...
    // Fill in header
    pblock->hashPrevBlock  = pindexPrev->GetBlockIDHash();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey, uint256 *powHash)
{
    uint256 hash = 0;
    ...
    // Found a solution
    {
        ...
        // Track how many getdata requests this block gets
        {
            ...
            wallet.mapRequestCount[pblock->GetIDHash()] = 0;
        }
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            ...
            if (pblock->IsProofOfStake())
            {
                ...
                strMintWarning = "";
                printf("CPUMiner : proof-of-stake block found %s\n", pblock->GetIDHash().ToString().c_str());
        ...
        loop
        {
            ...
            if (hash <= hashTarget)
            {
                ...
                strMintWarning = "";
                SetThreadPriority(THREAD_PRIORITY_NORMAL);
                CheckWork(pblock.get(), *pwalletMain, reservekey, &hash);
                SetThreadPriority(THREAD_PRIORITY_LOWEST);
                break;
            }
            ++pblock->nNonce;
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    if (fGenerate)
    {
        ...
        int nAddThreads = 1 - vnThreadsRunning[THREAD_MINER];
