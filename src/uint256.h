...
public:
    ...
    std::string GetHex(int nPadding=0) const
    {
        char psz[sizeof(pn)*2 + 1];
        for (unsigned int i = 0; i < sizeof(pn); i++)
            sprintf(psz + i*2, "%02x", ((unsigned char*)pn)[sizeof(pn) - i - 1]);
        std::string str(psz, psz + sizeof(pn)*2);
        while (str.length() < nPadding) {
            str = "0" + str;
        }
        return str;
    }
    ...
    std::string ToString(int nPadding=0) const
    {
        return (GetHex(nPadding));
    }
