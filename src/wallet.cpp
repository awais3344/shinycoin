...
bool CWallet::CreateTransaction(const vector<pair<CScript, int64> >& vecSend, const vector<pair<CBitcoinAddress, vector<CTxInfo> > >& vecTxInfos, CWalletTx& wtxNew, CReserveKey& reservekey, int64& nFeeRet)
{
    int64 nValue = 0;
    ...
    if (vecSend.empty() && vecTxInfos.empty())
        return false;
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                int64 nFeeLeft = nFeeRet;
                vector<const CWalletTx *> vecInfoTxFrom;
                BOOST_FOREACH(const PAIRTYPE(CBitcoinAddress, vector<CTxInfo>)& addr_info, vecTxInfos) {
                    if (!HaveKey(addr_info.first))
                        return false;
                    
                    bool fFound = false;
                    BOOST_FOREACH(const PAIRTYPE(const uint256, CWalletTx)& hash_tx, mapWallet) {
                        const CWalletTx *ptx = &hash_tx.second;
                        if (!ptx->IsFinal() || ptx->GetDepthInMainChain() < 1)
                            continue;
                        if (ptx->GetBlocksToMaturity() > 0)
                            continue;
                        
                        for (unsigned int i = 0; i < ptx->vout.size(); i++)
                        {
                            if (ptx->IsSpent(i) || !IsMine(ptx->vout[i]))
                                continue;
                            
                            if (ptx->nTime > wtxNew.nTime)
                                continue;
                            
                            CBitcoinAddress addr;
                            if (!ExtractAddress(ptx->vout[i].scriptPubKey, addr))
                                continue;
                            
                            if (addr == addr_info.first) {
                                CTxIn newIn(hash_tx.first, i);
                                newIn.infos = addr_info.second;
                                wtxNew.vin.push_back(newIn);
                                vecInfoTxFrom.push_back(ptx);
                                
                                int64 sendBack = ptx->vout[i].nValue;
                                if (sendBack >= nFeeLeft) {
                                    sendBack -= nFeeLeft;
                                    nFeeLeft = 0;
                                }
                                else if (sendBack > 0) {
                                ...
                                if (sendBack > 0) {
                                    ...
                                    scriptPubKey.SetBitcoinAddress(addr);
                                    wtxNew.vout.push_back(CTxOut(sendBack, scriptPubKey));
                                }
                ...
                assert(vecInfoTxFrom.size() == vecTxInfos.size());
                
                int nIn = 0;
                BOOST_FOREACH(const CWalletTx* ptx, vecInfoTxFrom)
                    if (!SignSignature(*this, *ptx, wtxNew, nIn++))
                        return false;
                ...
                // Choose coins to use
                int64 nTotalValue = nValue + nFeeLeft;
                ...
                int64 nChange = nValueIn - nValue - nFeeLeft;
...
bool CWallet::CreateTransaction(const vector<pair<CScript, int64> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64& nFeeRet) {
    return CreateTransaction(vecSend, vector<pair<CBitcoinAddress, vector<CTxInfo> > >(), wtxNew, reservekey, nFeeRet);
}
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64 nSearchInterval, CTransaction& txNew)
{
    ...
    int64 nCombineThreshold = GetNextProofOfWorkReward(pindexBest, true) / 3;
    ...
    int64 nReward;
    {
        ...
        CTxDB txdb("r");
        if (!txNew.GetCoinAge(txdb, nCoinAgeSeconds))
            return error("CreateCoinStake : failed to calculate coin age");
        nReward = GetProofOfStakeReward(nCoinAgeSeconds);
        if (nReward == 0) {
            if (fDebug && GetBoolArg("-printcoinstake"))
                printf("CreateCoinStake : would get zero stake reward\n");
            return false;
        }
        nCredit += nReward;
    }
    ...
    if (nReward <= nMinFee) {
        if (fDebug && GetBoolArg("-printcoinstake"))
            printf("CreateCoinStake : Reward %s would be less than fee %s\n", FormatMoney(nReward).c_str(), FormatMoney(nMinFee).c_str());
        return false;
    }
...
{
    {
        ...
        {
            CTxDB txdb("r");
            if (!mempool.checkAccept(txdb, wtxNew, true)) {
